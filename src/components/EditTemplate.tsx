...
export function EditTemplate({ template, onClose, onSuccess }: EditTemplateProps) {
  const { user } = useAuth();
  ...
  const [templateName, setTemplateName] = useState(template.template_name || template.name);
  ...
  const [headerMediaSample, setHeaderMediaSample] = useState(template.header_media_sample || '');
  const [headerMediaFilename, setHeaderMediaFilename] = useState(template.header_media_filename || '');
  ...
  const [buttons, setButtons] = useState<Button[]>(template.button_config?.buttons || []);

  const [catalogueId, setCatalogueId] = useState(template.catalogue_id || '');
  const [catalogueFormat, setCatalogueFormat] = useState(template.catalogue_format || 'full');

  const [validityPeriod, setValidityPeriod] = useState<number | null>(template.validity_period);
  const [useValidityPeriod, setUseValidityPeriod] = useState(!!template.validity_period);

  const [codeDeliveryMethod, setCodeDeliveryMethod] = useState(template.code_delivery_method || 'zero_tap');
  const [addSecurityRecommendation, setAddSecurityRecommendation] = useState(template.add_security_recommendation || false);
  const [codeExpiryMinutes, setCodeExpiryMinutes] = useState<number | null>(template.code_expiry_minutes);
  const [packageName, setPackageName] = useState(template.package_name || '');
  const [signatureHash, setSignatureHash] = useState(template.signature_hash || '');
  ...
  const [selectedCompanyName, setSelectedCompanyName] = useState<string | null>(template.company_name || null);
  ...
  const loadFolders = async () => {
    ...
    try {
      ...
      if (template.folder_id && data && data.length > 0) {
        const folder = data.find(f => f.id === template.folder_id);
        if (folder) {
          setSelectedFolderId(template.folder_id);
          setSelectedCompanyName(folder.company_name);
        }
      }
      
      // If no folder is set for this template but we have folders, 
